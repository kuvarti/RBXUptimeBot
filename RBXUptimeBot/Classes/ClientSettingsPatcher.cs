...
namespace RBXUptimeBot.Classes
{
    public static class ClientSettingsPatcher
    {
        public static void PatchSettings()
        {
            ...
            if (VersionFolder == null || !VersionFolder.Exists) { Logger.Error("Can't patch ClientAppSettings, folder doesn't exist"); return; }
            if (!VersionFolder.Name.StartsWith("version-")) { Logger.Error("Can't patch ClientAppSettings, folder doesn't start with 'version-'"); return; }
            if (!File.Exists(Path.Combine(VersionFolder.FullName, "RobloxPlayerLauncher.exe"))) { Logger.Error("Can't patch ClientAppSettings, RobloxPlayerBeta.exe not found"); return; }
