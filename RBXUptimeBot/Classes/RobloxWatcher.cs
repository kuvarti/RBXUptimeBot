...
namespace RBXUptimeBot.Classes
{
    internal class RobloxWatcher
    {
        ...
        public static System.Timers.Timer ReadTimer
        {
            get
            {
                if (readTimer == null)
                {
                    readTimer = new System.Timers.Timer(250);
        ...
        private static System.Timers.Timer readTimer;
        ...
        public static void CheckProcesses()
        {
            ...
            foreach (var process in Process.GetProcessesByName("RobloxPlayerBeta"))
            {
                ...
                void Kill(string Reason) { Logger.Information($"Attempting to kill process {process.Id}, reason: {Reason}"); try { process.Kill(); } catch { } }
                ...
                catch (Exception x) { Logger.Error($"Error with checking for Memory & Window Title: {x.Message}\n{x.StackTrace}"); }
                ...
                catch (Exception x) { Logger.Error($"Access to Process {process.Id} denied! This may be due to roblox being ran as admin or roblox's second process(This message can be ignored): {x.Message}"); }
            }
        }
